...
// export default function SavingsGoal() {
//   interface Goal {
//     goal: string;  // Expected to be a string representation of the goal amount
//     amount: string; // Expected to be a string representation of the achieved amount
//   }
...
//         },
//       });
//       setGoals(response.data.categories); // Make sure this is the correct path in your response
//     } catch (error) {
//       console.error('Error fetching goals:', error);
//       setError('Failed to fetch goals. Please try again later.'); // Set error message
//     } finally {
//       setFetching(false);
//     }
//   };
...
//   // Calculate the total goal and total amount achieved
//   const totalGoal = goals.reduce((sum, category) => sum + parseFloat(category.goal || '0'), 0);
//   const totalAchieved = goals.reduce((sum, category) => sum + parseFloat(category.amount || '0'), 0);
...
const SavingsGoal: React.FC<SavingsGoalProps> = ({ goals }) => {
  // Calculate the total goal and total amount achieved
  const totalGoal = goals.reduce((sum, category) => sum + parseFloat(category.goal || '0'), 0);
  const totalAchieved = goals.reduce((sum, category) => sum + parseFloat(category.amount || '0'), 0);
