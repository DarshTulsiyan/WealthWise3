...
//   const handleSaveGoal = async (goal: string, amount: number): Promise<void> => {
...
export default function WealthWise() {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [goals, setGoals] = useState<{ goal: string; amount: string }[]>([]); // Define the type of goals
  ...
  const handleSaveGoal = async (goal: string, amount: number): Promise<void> => {
    // Implement your logic to save the goal here
  };
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <main className="flex-1 p-8">
        ...
        <div className="grid grid-cols-2 gap-6 mb-6">
          <SavingsGoal goals={goals} /> {/* Pass goals prop here */}
          <SavingSummary />
        </div>
